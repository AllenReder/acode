...
async function createFixture(): Promise<{
  ...
  const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "awen-licenses-"));
...
describe("third-party license generation", () => {
  it("keeps the libghostty-vt notice pinned to its vendored license", async () => {
    const [config, license] = await Promise.all([
      ...
      NodeFSP.readFile(NodePath.join(REPOSITORY_ROOT, "native/libghostty-vt/LICENSE"), "utf8"),
    ]);

    expect(config).toContain('"name": "libghostty-vt"');
    expect(config).toContain('"noticeFile": "native/libghostty-vt/LICENSE"');
    expect(config).toContain('"sourceUrl": "https://github.com/ghostty-org/ghostty"');
