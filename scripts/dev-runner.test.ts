...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveDevHome", () => {
    it.effect("defaults to a checkout-local ACode directory", () =>
      Effect.sync(() => {
        assert.equal(
          resolveDevHome({
            checkoutRoot: "/work/acode",
    ...
    it.effect("honors an explicit home before checkout defaults", () =>
      Effect.sync(() => {
        assert.equal(
          resolveDevHome({
            checkoutRoot: "/work/acode",
            explicitHome: " /tmp/acode-test ",
            worktreeRoot: "/work/acode/.worktree",
            ambientHome: "/Users/example/.t3",
          }),
          "/tmp/acode-test",
        );
      }),
  ...
  describe("devPortProbeHosts", () => {
    ...
    // Only the backend honours --host/T3CODE_HOST. The Web server stays on
  ...
  describe("runDevRunnerWithInput", () => {
    ...
    // Base-dir precedence (--home-dir > worktree .acode > ambient T3CODE_HOME)
    ...
    describe("t3 home precedence", () => {
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".acode"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .acode over an ambient T3CODE_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("leaves the home implicit with no worktree and no ambient value", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.resolve(path.join(import.meta.dirname, "..", ".acode")));
        }),
      );
    });
  });
