...
import { DEV_PROXIED_PATH_PREFIXES } from "@awen/shared/devProxy";
...
const isSingleOriginDev = process.env.AWEN_SINGLE_ORIGIN_DEV === "1";
...
const sourcemapEnv = process.env.AWEN_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: AWEN_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); AWEN_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.AWEN_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // AWEN_PORT is set by scripts/dev-runner.ts for every dev mode.
  const port = Number(backendPort?.trim());
  if (Number.isInteger(port) && port > 0) {
    return `http://localhost:${port}/`;
  }

  // A separately configured backend still points the browser straight at the
  // daemon, so fall back to deriving the target from its websocket URL.
  if (!wsUrl) {
    return undefined;
  }
...
const devProxyTarget = resolveDevProxyTarget(process.env.AWEN_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.AWEN_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
  ...
  return {
    ...
    server: {
      ...
      ...(devProxyTarget
        ...
        : {}),
      // A caller that sets HOST needs the HMR socket pinned to that explicit
      // host to connect reliably. Leaving this unset lets the client derive it from the
