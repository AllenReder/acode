import { ComposerContextRecord, ForwardCompatibleArray } from "@awen/contracts";
...
export const PROMPT_STASH_STORAGE_KEY = "awen:prompt-stash:v2";
/**
 ...
 */
const LEGACY_PROMPT_STASH_STORAGE_KEY = "awen:prompt-stash:v1";
...
function resolveBaseStorage(): { storage: StateStorage; durable: boolean } {
  try {
    if (typeof localStorage !== "undefined") {
      // Node 25 exposes an empty localStorage object when it was started
      // without a valid --localstorage-file. Treat that shape like blocked
      // browser storage instead of calling methods that do not exist.
      const candidate = localStorage as unknown as Partial<StateStorage>;
      if (
