import { upgradeLegacyContextMessage } from "@awen/shared/composerContextLegacy";
...
describe("context reference paste", () => {
  it.each([
    ...
    ({ focus, prefix }) => {
      ...
      const importFragment = (
        fragment: import("@awen/contracts").ComposerContextClipboardFragment,
      ) => {
      ...
      const event = new TestClipboardEvent(
        `${prefix}![shot](awen-context://v1/image/img-old) and [T](awen-context://v1/terminal/ctx-t)`,
        {
          "web application/x-awen-context-fragment+json": JSON.stringify({
            version: 1,
      ...
      if (focus === "blurred") {
        expect(importPastedComposerText(event.clipboardData, importFragment)).toBe(
          `${prefix}![shot](awen-context://v1/image/img-new) and [T](awen-context://v1/terminal/ctx-t)`,
        );
  ...
  it("converts a copied legacy element into a sendable annotation and rewrites its link", () => {
    ...
    expect(text).toContain("awen-context://v1/preview-annotation/preview-annotation_imported");
  ...
  it("imports an annotation's dependent screenshot when only its chip is pasted", () => {
    ...
    const event = new TestClipboardEvent(
      `[Fix button](awen-context://v1/preview-annotation/${annotationId})`,
      {
        "web application/x-awen-context-fragment+json": JSON.stringify({
          version: 1,
          source: { environmentId: "env-1" },
  ...
  it("turns pasted context links into reference nodes", () => {
    ...
    pasteText(editor, "[Terminal 1 line 4](awen-context://v1/terminal/ctx-1) now");
    expect(editor.getEditorState().read(() => $getRoot().getTextContent())).toBe(
