const DIALOG_BACKDROP_BASE_CLASS =
  "fixed inset-0 z-50 transition-all [transition-duration:calc(200ms*var(--motion-duration-scale,1))] data-ending-style:opacity-0 data-starting-style:opacity-0";
...
const DIALOG_POPUP_BASE_CLASS =
  "-translate-y-[calc(1.25rem*var(--nested-dialogs))] relative flex min-h-0 w-full min-w-0 scale-[calc(1-0.1*var(--nested-dialogs))] flex-col opacity-[calc(1-0.1*var(--nested-dialogs))] outline-none transition-[scale,opacity,translate] [transition-duration:calc(200ms*var(--motion-duration-scale,1))] ease-in-out will-change-transform data-nested:data-ending-style:translate-y-8 data-nested:data-starting-style:translate-y-8 data-nested-dialog-open:origin-top data-ending-style:scale-98 data-starting-style:scale-98 data-ending-style:opacity-0 data-starting-style:opacity-0 [-webkit-app-region:no-drag]";

const DIALOG_POPUP_CLASS = `material-surface-overlay ${DIALOG_POPUP_BASE_CLASS} rounded-2xl border`;
