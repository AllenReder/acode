...
import type { AssistantCitation, MessageId, ScopedThreadRef } from "@awen/contracts";
...
const COMMENT_HIGHLIGHT_NAME = "awen-assistant-citation-comment";
...
export function observeAssistantCitationSource({
  ...
  const clear = () => {
    if (highlighted && CSS.highlights.get("awen-assistant-citation") === highlighted) {
      CSS.highlights.delete("awen-assistant-citation");
    }
    highlighted = null;
    ownedRange = null;
  ...
  const highlight = () => {
    ...
    if (typeof Highlight !== "undefined" && typeof CSS !== "undefined" && CSS.highlights) {
      ...
      CSS.highlights.set("awen-assistant-citation", highlighted);
      ownedRange = range;
