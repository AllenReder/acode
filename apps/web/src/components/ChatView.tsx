...
} from "@awen/shared/usageLimits";
import { feedbackBannerItem } from "./chat/ComposerFeedback";
import { usageLimitsBannerItem } from "./chat/ComposerUsageLimits";
import { derivePendingRequests } from "@awen/client-runtime/pending-requests";
import {
...
import { type EnvironmentConnectionPresentation } from "@awen/client-runtime/connection";
import { wasBootstrapThreadDeleted } from "@awen/client-runtime/errors";
import { readPastedComposerContext } from "./composerInlineTokenPaste";
import { isPasteAsTextShortcut } from "@awen/client-runtime/text-paste";
import { type CodexArtifactTemplate } from "@awen/client-runtime/codex-artifact-templates";
import { effectiveSnoozed, threadWokeAt } from "@awen/client-runtime/state/thread-settled";
import {
...
import { resolveProjectSettings } from "@awen/shared/projectSettings";
import { truncate } from "@awen/shared/String";
import { resolveThreadReferenceCopyTarget } from "@awen/shared/threadReference";
import {
  getTerminalLabel,
  nextTerminalId,
  resolveTerminalSessionLabel,
} from "@awen/shared/terminalLabels";
...
import { assistantCitationsToPlainText } from "@awen/shared/assistantCitations";
...
import { buildTemporaryWorktreeBranchName } from "@awen/shared/git";
...
} from "@awen/client-runtime/state/subagentRuntime";
...
import { serializeLegacyContextMessage } from "@awen/shared/composerContextLegacySend";
import {
  buildMessageContext,
...
import { requestOlderThreadTurns, threadHasOlderTurns } from "@awen/client-runtime/state/threads";
import { resolveProviderSkillsForCwd } from "@awen/client-runtime/providerSkills";
import { vcsEnvironment } from "../state/vcs";
import { sourceControlEnvironment } from "../state/sourceControl";
import { useProjectClone } from "../state/projectClones";
import { projectCloneDisplayName, projectCloneProgressSummary } from "@awen/contracts";
...
import { clampFileAttachmentUploadBytes } from "@awen/client-runtime/state/attachments";
...
const PersistentThreadTerminalDrawer = memo(function PersistentThreadTerminalDrawer({
  ...
  const workspaceId = project === null ? null : workspaceIdForAwenProject(project.id);
...
export default function ChatView(props: ChatViewProps) {
  ...
  const activeWorkspaceId =
    activeThread === undefined ? null : workspaceIdForAwenProject(activeThread.projectId);
  ...
  // Awen owns /usage-limits only where Limits has data for the selected provider;
  ...
  const [resumeCompactionPermanentlyDismissed, setResumeCompactionPermanentlyDismissed] =
    useLocalStorage(
      `awen:resume-compaction-dismissed:${environmentId}:${activeProviderInstanceId ?? "claudeAgent"}`,
      false,
      Schema.Boolean,
    );
  ...
  // the sent message can stay near the top. Awen only owns streaming adjustments
  ...
  const onRespondToUserInput = useCallback(
    async (requestId: ApprovalRequestId, answers: Record<string, unknown>) => {
      ...
      const attachmentsByQuestionId = new Map<
        string,
        import("@awen/contracts").UserInputAttachments[string]
      >();
      for (const question of activePendingUserInput.questions) {
        ...
        attachmentsByQuestionId.set(
          question.id,
          uploaded as import("@awen/contracts").UserInputAttachments[string],
        );
      }
  ...
  const panelToggleControls = workbenchMode ? null : (
    <PanelLayoutControls
  ...
  const panelLayoutControls = workbenchMode ? null : (
    <div
      className={cn(
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Awen server to browse pull requests."
      />
  ...
  return (
    <div data-chat-view-root="" className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden">
      ...
      {!workbenchMode && rightPanelPresent && !shouldUseRightPanelSheet && activeThreadRef ? (
        <RightPanelTabs
          mode="inline"
          widthStorageKey={`awen:preview-panel-width:${activeThreadKey}`}
          open={rightPanelOpen}
          maximized={rightPanelMaximized}
