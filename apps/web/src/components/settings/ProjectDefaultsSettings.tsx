...
export function ProjectDefaultsSettings({ category }: { category: ProjectSettingsCategory }) {
  ...
  const repositoryEnvMode = awenFile.file?.defaultThreadEnvMode ?? null;
  const inheritedEnvModeLabel =
    workspaceSource === "project"
      ? null
      : repositoryEnvMode
        ? `${resolveEnvModeLabel(repositoryEnvMode)} (awen.json)`
        : null;
      ...
      {category === "general" ? (
        <>
          ...
          <SettingsRow
            ...
            description={
              isProjectScope
                ? "Where new threads in this project start. A awen.json preference applies when the project has no override."
                : "Where new threads start, unless overridden by the project or awen.json."
            }
            status={
