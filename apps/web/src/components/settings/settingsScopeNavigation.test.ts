import { EnvironmentId, ProviderInstanceId } from "@awen/contracts";
import {
  createMemoryHistory,
  createRootRoute,
  createRoute,
...
const checkoutSearch = {
  project: "repository:awen",
  machine: "remote-server",
  checkout: "remote-server:/home/user/Awen",
};
...
describe("settings scope navigation", () => {
  ...
  it.each(["/settings/projects", "/settings/integrations", "/settings/source-control"] as const)(
    ...
    async (to) => {
      ...
      const regroupedCheckout = { ...checkoutSearch, project: "separate:awen" };
      await router.navigate({
        from: "/settings",
