import { EnvironmentId, type AwenProjectFileScript } from "@awen/contracts";
import {
  isAtomCommandInterrupted,
  squashAtomCommandFailure,
} from "@awen/client-runtime/state/runtime";
import { DEFAULT_RESOLVED_KEYBINDINGS } from "@awen/shared/keybindings";
...
export function ProjectActionsSettings() {
  ...
  // A project's awen.json can declare actions to import. Read it from the
  ...
  const importableScripts = useMemo(
    ...
    [scripts, awenFile.scripts],
  );
  const importFileScript = useCallback(
    async (fileScript: AwenProjectFileScript) => {
  ...
  return (
    <SettingsSection id="project-actions" title="Actions">
            ...
            {importableScripts.length > 0 ? (
              <Menu>
                ...
                <MenuPopup align="end" className="w-72">
                  <MenuGroup>
                    <MenuGroupLabel>Import from awen.json</MenuGroupLabel>
      ...
      {awenFile.status === "invalid" ? (
        <SettingsRow
          title="awen.json is invalid"
          description="A awen.json exists in this checkout but fails to parse, so every action and icon it declares is ignored. Check the JSON syntax and icon values."
