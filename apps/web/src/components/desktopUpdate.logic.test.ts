...
describe("desktop update UI helpers", () => {
  it("builds the stable release URL for a downloaded version", () => {
    expect(getDesktopUpdateReleaseUrl("0.0.30")).toBe(
      "https://github.com/AllenReder/awen/releases/tag/v0.0.30",
    );
  });

  it("builds a prerelease URL without dropping its version suffix", () => {
    expect(getDesktopUpdateReleaseUrl("0.1.0-alpha.1")).toBe(
  ...
  it("builds the release history URL", () => {
    expect(getDesktopUpdateReleaseHistoryUrl()).toBe("https://github.com/AllenReder/awen/releases");
  });
  ...
  it("includes the downloaded version in the install confirmation copy", () => {
    ...
    ).toContain("Install update 1.1.1 and restart Awen?");
  });
  ...
  it("falls back to generic install confirmation copy when no version is available", () => {
    ...
    ).toContain("Install update and restart Awen?");
  });
  ...
  it("keeps the same install confirmation copy across desktop platforms", () => {
    expect(
      ...
      "Install update 1.1.0 and restart Awen?\n\nAny running tasks will be interrupted. Make sure you're ready before continuing.",
    );
  });
});
