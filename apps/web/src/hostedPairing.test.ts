...
describe("hostedPairing", () => {
  ...
  it("reads hosted pairing host and query token parameters", () => {
    const url = new URL("https://app.awen.example/pair?host=100.64.1.2:3773&token=ABCD1234");
  ...
  it("prefers hash tokens so generated hosted links do not put credentials in search params", () => {
    vi.stubEnv("VITE_HOSTED_APP_URL", "https://preview.awen.example");
    ...
    expect(url.origin).toBe("https://preview.awen.example");
  ...
  it("ignores incomplete hosted pairing requests", () => {
    expect(
      hasHostedPairingRequest(new URL("https://app.awen.example/pair?host=backend.example.com")),
    ).toBe(false);
    expect(hasHostedPairingRequest(new URL("https://app.awen.example/pair?token=ABCD1234"))).toBe(
      false,
    );
  });
  ...
  it("detects the hosted static app only when no backend URL is configured", () => {
    ...
    expect(isHostedStaticApp(new URL("https://preview.awen.example/"))).toBe(true);
    expect(isHostedStaticApp(new URL("https://preview.awen.example/pair"))).toBe(true);
    expect(isHostedStaticApp(new URL("https://backend.example.com/"))).toBe(false);

    vi.stubEnv("VITE_HTTP_URL", "https://backend.example.com");
    expect(isHostedStaticApp(new URL("https://preview.awen.example/"))).toBe(false);
  });
});
