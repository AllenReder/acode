...
import { VIDEO_FILE_EXTENSIONS } from "@awen/shared/video";
...
const PIERRE_ICON_SPRITE_ID = "awen-pierre-file-icon-sprite";

const AWEN_FILE_ICON_SPRITE = `
<svg xmlns="http://www.w3.org/2000/svg" width="0" height="0" aria-hidden="true">
  <!-- Lucide Film icon, ISC license. -->
  <symbol id="awen-file-icon-video" viewBox="0 0 24 24">
  ...
  <symbol id="awen-file-icon-agents" viewBox="0 0 32 32">
  ...
  <symbol id="awen-file-icon-pnpm" viewBox="0 0 32 32">
...
export const AWEN_PIERRE_ICONS = {
  set: "complete",
  colored: true,
  spriteSheet: AWEN_FILE_ICON_SPRITE,
  byFileName: {
    "package.json": "file-tree-builtin-npm",
    "tsconfig.json": "file-tree-builtin-typescript",
    "agents.md": "awen-file-icon-agents",
    "pnpm-lock.yaml": "awen-file-icon-pnpm",
    "pnpm-workspace.yaml": "awen-file-icon-pnpm",
  },
  byFileExtension: Object.fromEntries(
    VIDEO_FILE_EXTENSIONS.map((extension) => [extension, "awen-file-icon-video"]),
  ),
} satisfies FileTreeIcons;

const completeIconResolver = createFileTreeIconResolver(AWEN_PIERRE_ICONS);
...
export function ensurePierreIconSprite(): void {
  ...
  container.innerHTML = `${getBuiltInSpriteSheet("complete")}${AWEN_FILE_ICON_SPRITE}`;
