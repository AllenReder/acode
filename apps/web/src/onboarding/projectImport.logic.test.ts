import { EnvironmentId, ProjectId, type AgentSessionProjectCandidate } from "@awen/contracts";
...
describe("groupOnboardingProjects", () => {
  it("groups clones by origin, keeps local repos separate, and folds non-git folders away", () => {
    const main = candidate("/code/awen", {
      git: github("AllenReder/awen"),
      threadCount: 79,
      lastActiveAt: "2026-08-21T12:00:00.000Z",
    });
    const clone = candidate("/code/clones/awen-2", {
      git: github("AllenReder/awen"),
      threadCount: 13,
      lastActiveAt: "2026-08-10T12:00:00.000Z",
    });
    const older = candidate("/code/fleet", {
      git: github("awendotgg/fleet"),
      threadCount: 295,
    ...
    expect(
      ...
      {
        label: "awendotgg/fleet",
        paths: ["/code/fleet"],
        threadCount: 295,
        lastActiveAt: "2026-08-22T00:00:00.000Z",
      },
      {
        label: "AllenReder/awen",
        paths: ["/code/awen", "/code/clones/awen-2"],
        threadCount: 92,
        lastActiveAt: "2026-08-21T12:00:00.000Z",
      },
      {
