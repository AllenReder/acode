...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with Awen-specific exact filename icons", () => {
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "awen-file-icon-agents");
    assert.equal(resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name, "awen-file-icon-pnpm");
    assert.equal(
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(
      Object.values(AWEN_PIERRE_ICONS.byFileName).filter((name) => name.startsWith("awen-")),
    );
    for (const iconName of customIconNames) {
      assert.include(AWEN_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
