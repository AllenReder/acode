...
import { ProjectId, type RepositoryIdentity } from "@awen/contracts";
...
describe("pullRequestCandidateUrlFromReferenceAutolink", () => {
  it("turns GitHub's shared issue route into a pull request candidate", () => {
    expect(
      pullRequestCandidateUrlFromReferenceAutolink(
        "https://github.com/AllenReder/awen/issues/8600#issuecomment-1",
      ),
    ).toBe("https://github.com/AllenReder/awen/pull/8600#issuecomment-1");
  });

  it("does not reinterpret other issue hosts or malformed references", () => {
    expect(
      pullRequestCandidateUrlFromReferenceAutolink(
        "https://gitlab.com/AllenReder/awen/-/issues/8600",
      ),
    ).toBeNull();
    expect(
      pullRequestCandidateUrlFromReferenceAutolink(
        "https://github.com/AllenReder/awen/issues/not-a-number",
      ),
    ).toBeNull();
  });
});
...
describe("matchesLinkedPullRequestUrl", () => {
  ...
  it("matches the same pull request without looking up its project", () => {
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.com/AllenReder/awen/pull/42/files",
      ),
    ).toBe(true);
  });
  ...
  it("keeps other providers' existing port normalization", () => {
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.com:8443/AllenReder/awen/pull/42",
      ),
    ).toBe(true);
  });
  ...
  it("rejects a different pull request or host", () => {
    expect(
      matchesLinkedPullRequestUrl(linkedPullRequest, "https://github.com/AllenReder/awen/pull/43"),
    ).toBe(false);
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.example.com/AllenReder/awen/pull/42",
      ),
    ).toBe(false);
  });
});
...
describe("parseChangeRequestUrl", () => {
  it("reads a GitHub pull request", () => {
    expect(parseChangeRequestUrl("https://github.com/allenreder/awen/pull/123")).toEqual({
      host: "github.com",
      repository: "allenreder/awen",
      number: 123,
    });
  ...
  it("reads a GitLab merge request, nested groups and all", () => {
    expect(
      parseChangeRequestUrl("https://gitlab.com/allenreder/platform/awen/-/merge_requests/42"),
    ).toEqual({
      host: "gitlab.com",
      repository: "allenreder/platform/awen",
      number: 42,
    });
  });
  ...
  it("reads both Azure DevOps URL forms, keeping `_git` in the repository path", () => {
    expect(
      parseChangeRequestUrl("https://dev.azure.com/acme/platform/_git/awen/pullrequest/17"),
    ).toEqual({
      host: "dev.azure.com",
      repository: "acme/platform/_git/awen",
      number: 17,
    });
    expect(
      parseChangeRequestUrl("https://acme.visualstudio.com/platform/_git/awen/pullrequest/17"),
    ).toEqual({
      host: "acme.visualstudio.com",
      repository: "platform/_git/awen",
      number: 17,
    });
  });

  it("survives trailing segments, a trailing slash and a query string", () => {
    expect(parseChangeRequestUrl("https://github.com/allenreder/awen/pull/123/files?w=1")).toEqual({
      host: "github.com",
      repository: "allenreder/awen",
      number: 123,
    });
  ...
  it("claims nothing it cannot be sure of, so the link goes to the browser", () => {
    for (const link of [
      "https://github.com/allenreder/awen/issues/123",
      "https://github.com/allenreder/awen/commit/0a1b2c3",
      "https://github.com/allenreder/awen",
      "https://github.com/allenreder/awen/pull/abc",
      "https://gitlab.com/allenreder/awen/-/snippets/12",
      "https://gitlab.com/allenreder/awen/-/issues/12",
      ...
      "javascript:alert(1)//github.com/allenreder/awen/pull/1",
      "not a url",
    ]) {
...
describe("findProjectForChangeRequest", () => {
  ...
  it("matches a nested GitLab group by the whole path below the host", () => {
    ...
    // two-segment owner/name form would look for `allenreder/awen` and find nothing.
    const projects = [
      project({
        canonicalKey: "gitlab.com/allenreder/platform/awen",
        provider: "gitlab",
        displayName: "allenreder/platform/awen",
        owner: "allenreder",
        name: "awen",
      }),
    ];
  ...
  it("keeps two hosts apart, so an Enterprise link does not open the public one", () => {
    const projects = [
      project({
        canonicalKey: "github.com/AllenReder/awen",
        provider: "github",
        owner: "pingdotgg",
        name: "awen",
      }),
