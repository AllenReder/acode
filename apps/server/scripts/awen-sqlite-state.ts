...
import { fromJsonStringPretty } from "@awen/shared/schemaJson";
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedError<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Awen once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedError<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.awen database. Use an isolated --base-dir.";
  }
}
...
const awenSqliteStateCommand = Command.make(
  "awen-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Awen data directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Awen SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(awenSqliteStateCommand, { version: "0.0.0" }).pipe(
