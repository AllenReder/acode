...
const buildExeCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
        ...
        ChildProcess.make(spawnCommand.command, spawnCommand.args, {
          ...
          env: {
            ...
            ...Option.match(config.target, {
              onNone: () => ({}),
              onSome: (target) => ({ AWEN_PACK_EXE_TARGET: target }),
            }),
          },
          stdout: config.verbose ? "inherit" : "ignore",
          stderr: "inherit",
      ...
      yield* Effect.log(
        "[cli] Built dist-exe/awen (expects client/, resource-monitor/, and the runtime-external node_modules beside it; scripts/build-cli-archive.ts assembles that tree)",
      );
    }),
).pipe(
 ...
 * every `@awen/awen-<platform>.tgz` first, `awen.tgz` (the launcher) last, so
...
const publishCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const scopeDir = path.join(packagesDir, "@awen");
      const launcherTarball = path.join(packagesDir, "awen.tgz");
      const platformTarballs = (yield* fs
        .readDirectory(scopeDir)
        .pipe(Effect.orElseSucceed((): ReadonlyArray<string> => [])))
        .filter((entry) => entry.startsWith("awen-") && entry.endsWith(".tgz"))
        .sort()
        .map((entry) => path.join(scopeDir, entry));
      if (platformTarballs.length === 0) {
        return yield* new ServerCliBuildAssetMissingError({
          assetPath: path.join(scopeDir, "awen-<platform>.tgz"),
        });
      }
  ...
  Command.withDescription(
    "Publish the @awen/awen-<platform> tarballs and then the awen launcher to npm.",
  ),
);
...
const cli = Command.make("cli").pipe(
  Command.withDescription("Awen server build & publish CLI."),
