...
import { HostProcessWorkingDirectory } from "@awen/shared/hostProcess";
...
import { VcsProcessExitError, VcsProcessSpawnError, VcsProcessTimeoutError } from "@awen/contracts";
  ...
  it.effect("retains spawn causes without exposing process arguments in the error message", () =>
    Effect.gen(function* () {
      ...
      const error = yield* run({
        ...
        command: "definitely-not-a-awen-executable",
        args: [secretArgument],
        cwd: process.cwd(),
      }).pipe(Effect.flip);
