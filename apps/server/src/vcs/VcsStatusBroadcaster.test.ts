...
import { GitManagerError } from "@awen/contracts";
...
const remoteStatusWithPr: VcsStatusRemoteResult = {
  ...
  pr: {
    ...
    url: "https://github.com/AllenReder/awen/pull/2978",
    baseRef: "main",
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const realDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "awen-vcs-auto-pull-real-",
        });
        const linkParent = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "awen-vcs-auto-pull-link-",
        });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const realDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "awen-vcs-status-real-",
        });
        const linkParent = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "awen-vcs-status-link-",
        });
