...
import { EnvironmentHttpApi } from "@awen/contracts";
...
).pipe(Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "awen-auth-http-test-" })));
    ...
    return yield* Effect.acquireUseRelease(
      ...
      ([environmentA, environmentB]) =>
        Effect.tryPromise(async () => {
          ...
          const devCookie = devCookies.find((cookie) => cookie.startsWith("awen_dev_session_"));
          expect(devCookie).toContain("HttpOnly");
          expect(devCookie).toContain(`=${DEV_TOKEN};`);
          expect(devCookies).toContainEqual(
            expect.stringMatching(/^awen_session_[^=]*=;.*Max-Age=0/),
          );
          ...
          expect(restrictedCookies[0]).toMatch(/^awen_session_/);
          expect(restrictedCookies[0]).not.toContain("awen_dev_session_");
        }),
      ([environmentA, environmentB]) =>
