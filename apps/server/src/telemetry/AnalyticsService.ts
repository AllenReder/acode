...
import type { ClientOs } from "@awen/contracts";
...
const TelemetryEnvConfig = Config.all({
  posthogKey: Config.string("AWEN_POSTHOG_KEY").pipe(
    Config.withDefault("phc_XOWci4oZP4VvLiEyrFqkFjP4CZn55mjYYBMREK5Wd6m"),
  ),
  posthogHost: Config.string("AWEN_POSTHOG_HOST").pipe(
    Config.withDefault("https://us.i.posthog.com"),
  ),
  enabled: Config.boolean("AWEN_TELEMETRY_ENABLED").pipe(Config.withDefault(true)),
  flushBatchSize: Config.number("AWEN_TELEMETRY_FLUSH_BATCH_SIZE").pipe(Config.withDefault(20)),
  maxBufferedEvents: Config.number("AWEN_TELEMETRY_MAX_BUFFERED_EVENTS").pipe(
...
>()("@awen/server/telemetry/AnalyticsService") {
    ...
    const payload = {
      ...
      batch: events.map((event) => ({
        ...
        properties: {
          ...
          awenVersion: packageJson.version,
          clientType,
