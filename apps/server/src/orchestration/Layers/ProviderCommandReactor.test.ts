...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir =
      input?.baseDir ?? NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "awen-reactor-"));
    ...
    Effect.gen(function* () {
      ...
      yield* harness.engine.dispatch({
        ...
        message: {
          ...
          role: "user",
          text: "Inspect [build](awen-context://v1/terminal/terminal-1)",
          attachments: [],
          context: {
            version: 1,
            records: [
  ...
  it("pins the first user message when regeneration context is truncated", async () => {
    ...
    expect(message).not.toContain("awen-citation://");
  ...
  it("generates a worktree branch name for the first turn", async () => {
    ...
    expect(harness.generateBranchName.mock.calls[0]?.[0].message).not.toContain("awen-citation://");
