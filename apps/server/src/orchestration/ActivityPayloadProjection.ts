import { projectQuestionToolInput } from "@awen/shared/toolActivity";
import type {
  OrchestrationEvent,
...
import { extractJsonObject } from "@awen/shared/schemaJson";
...
function projectPreviewToolMetadata(data: Record<string, unknown>, status: unknown) {
  ...
  if (
    typeof name !== "string" ||
    !/^(?:mcp__)?(?:awen|awen|awen)_{1,2}preview_(?:open|navigate|status|snapshot|click|type|press|scroll|resize|set_appearance|evaluate|wait_for|recording_start|recording_stop)$/.test(
      name,
    )
  )
    return {};
