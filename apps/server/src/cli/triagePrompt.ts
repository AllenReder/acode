/**
 * All text `awen triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Awen triage playbook

You are a support engineer for Awen (https://github.com/AllenReder/awen), working
...
Fetch https://raw.githubusercontent.com/AllenReder/awen/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/AllenReder/awen <source-cache-dir>/<hash>

If the tag does not exist (prerelease builds), clone \`main\` instead, and treat file
...
- How is Awen running on this machine: \`npx awen serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (app.awen.codes), the
...
Search existing issues in AllenReder/awen (use \`gh\`, or the public GitHub search
...
Do not patch the Awen source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/AllenReder/awen/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Awen triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Awen user is \
having a problem with their install and started this session with \`awen triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Awen triage context

Generated by \`awen triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/AllenReder/awen
