...
it.layer(NodeServices.layer)("awen update launcher", (it) => {
  it.effect("repoints a symlink that lives in a runtime versions tree", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "awen-update-" });
      const oldExe = path.join(root, "runtime/versions/1.0.0/awen");
      const newExe = path.join(root, "runtime/versions/2.0.0/awen");
      const launcher = path.join(root, "bin/awen");
  ...
  it.effect("leaves a plain copy or a foreign symlink alone", () =>
    Effect.gen(function* () {
      ...
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "awen-update-" });
      const newExe = path.join(root, "runtime/versions/2.0.0/awen");
      const copy = path.join(root, "copy/awen");
      const foreign = path.join(root, "foreign/awen");
      const elsewhere = path.join(root, "elsewhere/awen");
      // Another install's versions tree: same shape, different home.
      const otherHome = path.join(root, "other/runtime/versions/1.0.0/awen");
      const otherLauncher = path.join(root, "other/bin/awen");
  ...
  it.effect("finds the launcher a bare command name resolved to on PATH", () =>
    Effect.gen(function* () {
      ...
      const bare = yield* resolveLauncherPath.pipe(
        Effect.provideService(HostProcessInvokedAs, "awen"),
      ...
      const relative = yield* resolveLauncherPath.pipe(
        Effect.provideService(HostProcessInvokedAs, "./bin/awen"),
