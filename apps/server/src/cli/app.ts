...
export class DesktopAppSshUnsupportedError extends Schema.TaggedError<DesktopAppSshUnsupportedError>()(
  ...
  override get message(): string {
    return "`awen app` only controls a desktop app on the same machine. It cannot run over SSH.";
  }
}
...
export class DesktopAppPlatformUnsupportedError extends Schema.TaggedError<DesktopAppPlatformUnsupportedError>()(
  ...
  override get message(): string {
    return `\`awen app\` is not supported on ${this.platform}.`;
  }
}
...
export class DesktopAppUnreachableError extends Schema.TaggedError<DesktopAppUnreachableError>()(
  ...
  override get message(): string {
    return "Could not reach the Awen desktop app. Start or update the desktop app on this machine, then run `awen app` again. A running Awen server is not enough.";
  }
}
...
export class DesktopAppRequestFailedError extends Schema.TaggedError<DesktopAppRequestFailedError>()(
  ...
  override get message(): string {
    return `Awen could not open ${this.workspaceRoot} (${this.code}).`;
  }
}
...
const runAppCommand = Effect.fn("cli.app")(function* (flags: {
  ...
  const configuredBaseDir = Option.getOrUndefined(flags.baseDir) ?? environment.awenHome;
  const baseDir = yield* resolveBaseDir(configuredBaseDir);
  const allowDevFallback = Option.isNone(flags.baseDir) && !environment.awenHome?.trim();
  ...
  yield* Console.log(`Opened ${workspaceRoot} in Awen.`);
});
...
export const appCommand = Command.make("app", {
  ...
  Command.withDescription("Open a project in the running Awen desktop app."),
