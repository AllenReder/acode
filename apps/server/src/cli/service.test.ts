...
const status = {
  ...
  installed: true,
  current: true,
  unitPath: "/home/me/.config/systemd/user/awen.service",
  logPath: "/home/me/.awen/userdata/logs/boot-service.log",
} as const;
...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "Awen service",
      "  Status: installed · awen@0.0.29",
      "  Unit: /home/me/.config/systemd/user/awen.service",
      "  Logs: /home/me/.awen/userdata/logs/boot-service.log",
    ].join("\n"),
  );
});
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `awen service install` to repair it.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("Run `awen service install` to repair it.");
...
it("reports a newer installed service and tells the CLI to catch up to it", () => {
  ...
  assert.include(output, "awen@0.0.32-nightly.1 (newer than this awen@0.0.31 CLI)");
  assert.include(output, "Run `awen update 0.0.32-nightly.1` to match it");
  ...
  const service = BootService.BootService.of({
    ...
    install: (options) =>
      Effect.sync(() => {
        ...
        return {
          program: ["/test/awen/runtime/versions/1.0.0/awen", "__service-launcher"],
          baseDir: "/test/awen",
...
it.layer(Layer.mergeAll(NodeServices.layer, NetService.layer))("service commands", (it) => {
  it.effect("restart restarts the installed service", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "awen-service-cli-test-" });
