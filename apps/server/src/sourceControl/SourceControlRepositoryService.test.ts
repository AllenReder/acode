...
import { GitCommandError, SourceControlProviderError } from "@awen/contracts";
...
const CLONE_URLS = {
  nameWithOwner: "octocat/awen",
  url: "https://github.com/octocat/awen",
  sshUrl: "git@github.com:octocat/awen.git",
};
  ...
  const serviceLayer = SourceControlRepositoryService.layer.pipe(
    ...
    Layer.provide(
      ServerConfig.layerTest(
        process.cwd(),
        input.fileSystem
          ? "/tmp/awen-source-control-repos"
          : { prefix: "awen-source-control-repos-" },
      ),
    ),
  );
...
it.effect("looks up repositories through the requested provider without search", () => {
  ...
  return Effect.gen(function* () {
    ...
    const result = yield* service.lookupRepository({
      provider: "github",
      repository: "octocat/awen",
      cwd: "/workspace",
    });

    assert.deepStrictEqual(result, { provider: "github", ...CLONE_URLS });
    assert.deepStrictEqual(calls, [{ cwd: "/workspace", repository: "octocat/awen" }]);
...
it.effect("clones a looked-up repository into the requested destination", () =>
  Effect.gen(function* () {
    ...
    const parent = yield* fs.makeTempDirectoryScoped({
      prefix: "awen-source-control-clone-parent-",
    });
    const destinationPath = path.join(parent, "awen");
    ...
    yield* Effect.gen(function* () {
      ...
      assert.deepStrictEqual(cloneCalls, [
        {
          cwd: parent,
          args: ["clone", "--progress", CLONE_URLS.url, "awen"],
        },
      ]);
    }).pipe(
      Effect.provide(
...
it.effect("reports clone progress from git's stderr and keeps its error text on failure", () =>
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    const parent = yield* fs.makeTempDirectoryScoped({
      prefix: "awen-source-control-clone-progress-",
    });
    const destinationPath = path.join(parent, "awen");
    ...
    const stderrLines = [
      ...
      "fatal: early EOF",
      "fatal: unable to access 'https://user:s3c@ret@github.com/octocat/awen.git/': could not resolve host",
    ];
    ...
    assert.strictEqual(
      error.detail,
      "fatal: early EOF fatal: unable to access 'https://github.com/octocat/awen.git/': could not resolve host",
    );
  }).pipe(Effect.provide(NodeServices.layer)),
);
...
it.effect("strips embedded credentials from the remote URL it reports", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const parent = yield* fs.makeTempDirectoryScoped({ prefix: "awen-source-control-redact-" });
    const destinationPath = path.join(parent, "awen");
    const cloneArgs: Array<ReadonlyArray<string>> = [];
    const result = yield* Effect.gen(function* () {
      const service = yield* SourceControlRepositoryService.SourceControlRepositoryService;
      return yield* service.prepareClone({
        remoteUrl: "https://user:s3cret@github.com/octocat/awen.git",
        destinationPath,
      });
    }).pipe(
      Effect.provide(
    ...
    assert.equal(result.remoteUrl, "https://github.com/octocat/awen.git");
    // Git itself still receives the credentials.
    assert.equal(result.cloneUrl, "https://user:s3cret@github.com/octocat/awen.git");
  }).pipe(Effect.provide(NodeServices.layer)),
);
...
it.effect("discards only a directory git wrote to", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const parent = yield* fs.makeTempDirectoryScoped({ prefix: "awen-source-control-discard-" });
...
it.effect("redacts query tokens and userinfo containing '@' from reported URLs", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const parent = yield* fs.makeTempDirectoryScoped({ prefix: "awen-source-control-redact2-" });
    yield* Effect.gen(function* () {
      const service = yield* SourceControlRepositoryService.SourceControlRepositoryService;
      const query = yield* service.prepareClone({
        remoteUrl: "https://github.com/octocat/awen.git?access_token=s3cret",
        destinationPath: path.join(parent, "a"),
      });
      assert.equal(query.remoteUrl, "https://github.com/octocat/awen.git");
      const nested = yield* service.prepareClone({
        remoteUrl: "https://user:pa@rt@github.com/octocat/awen.git",
        destinationPath: path.join(parent, "b"),
      });
      assert.equal(nested.remoteUrl, "https://github.com/octocat/awen.git");
    }).pipe(Effect.provide(makeLayer({})));
  }).pipe(Effect.provide(NodeServices.layer)),
);
...
it.effect("preserves destination probe failures instead of treating them as missing paths", () => {
  const fileSystemCause = PlatformError.systemError({
    ...
    module: "FileSystem",
    method: "exists",
    pathOrDescriptor: "/restricted/awen",
  });
  ...
  return Effect.gen(function* () {
    ...
    const error = yield* Effect.flip(
      service.cloneRepository({
        ...
        destinationPath: "/restricted/awen",
      }),
    );
...
it.effect("publishes by creating the repository, adding a remote, and pushing upstream", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(createCalls, [
      { cwd: "/workspace", repository: "octocat/awen", visibility: "private" },
    ]);
