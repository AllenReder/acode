...
import { sanitizeBranchFragment } from "@awen/shared/git";
import {
  detectSourceControlProviderFromRemoteUrl,
  isSshRemoteUrl,
} from "@awen/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("AWEN_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("AWEN_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("AWEN_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("AWEN_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("@awen/server/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `awen/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set AWEN_BITBUCKET_EMAIL and AWEN_BITBUCKET_API_TOKEN, or AWEN_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
