...
// Opt in with AWEN_TEST_OPENCODE_V2_BINARY=/path/to/opencode. All model calls stay on loopback.
const binaryPath = process.env.AWEN_TEST_OPENCODE_V2_BINARY;
...
describe.runIf(Boolean(binaryPath))("OpenCode v2 native API", () => {
  it.live("runs chat, approval, fork and compaction against the installed binary", () =>
    Effect.gen(function* () {
      ...
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "awen-opencode-native-" });
      ...
      NodeAssert.ok(server.serverPassword, "managed v2 server must generate a password");
      const unauthenticated = runtime.createOpenCodeSdkClient({ baseUrl: server.url });
      yield* Effect.promise(() =>
        NodeAssert.rejects(
          () => unauthenticated.server.info(),
          "an unauthenticated request must be rejected by the v2 server",
        ),
      );
