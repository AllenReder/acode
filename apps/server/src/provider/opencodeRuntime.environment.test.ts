  ...
  effectIt.effect("rejects a server below the supported version", () =>
    Effect.gen(function* () {
      const error = yield* verifyOpenCodeServerVersion(
        makeHealthClient(() => Promise.resolve({ version: "2.0.11" })),
      ).pipe(Effect.flip);
      expect(error).toBeInstanceOf(OpenCodeRuntimeError);
      expect(error.detail).toContain("v2.0.11 is too old");
    }),
  );
  ...
  effectIt.live(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "awen-opencode-output-" });
...
`,
        ...
        yield* fs.writeFileString(
          ...
          [
            ...
            isWindows
              ? '"%AWEN_TEST_NODE_BINARY%" "%AWEN_TEST_OPENCODE_SCRIPT%" %*'
              : 'exec "$AWEN_TEST_NODE_BINARY" "$AWEN_TEST_OPENCODE_SCRIPT" "$@"',
            "",
          ].join("\n"),
        );
        if (!isWindows) {
        ...
        const server = yield* runtime.startOpenCodeServerProcess({
          ...
          environment: {
            ...environment,
            AWEN_TEST_NODE_BINARY: executablePath,
            AWEN_TEST_OPENCODE_SCRIPT: scriptPath,
          },
        });
