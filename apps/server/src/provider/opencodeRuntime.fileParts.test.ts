...
describe("toOpenCodeFileParts", () => {
  it("maps supported attachments to file URIs with names", () => {
    ...
    NodeAssert.deepEqual(parts, [
      { uri: NodeURL.pathToFileURL("/attachments/shot.png").href, name: "shot.png" },
      { uri: NodeURL.pathToFileURL("/attachments/notes.txt").href, name: "notes.txt" },
      { uri: NodeURL.pathToFileURL("/attachments/spec.pdf").href, name: "spec.pdf" },
    ]);
  });
