...
import { parseCliArgs } from "@awen/shared/cliArgs";
...
} from "@awen/shared/claudeCompaction";
import { HostProcessIsExecutable } from "@awen/shared/hostProcess";
...
// conversation messages from the truncated end, then remap Awen turn starts.
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleSystemMessage = Effect.fn("handleSystemMessage")(function* (
    ...
    switch (message.subtype) {
      ...
      case "model_refusal_fallback":
        ...
        return;
      // Inner protocol/UX details with no Awen surface today — consumed
  ...
  const handleSdkMessage = Effect.fn("handleSdkMessage")(function* (
    ...
    // Wire-only command bookkeeping has no user-facing Awen lifecycle.
    ...
    switch (message.type) {
      ...
      case "rate_limit_event":
        ...
        return;
      // Composer prompt suggestions have no Awen surface; consumed deliberately.
      // `conversation_reset` announces a CLI-side conversation id swap
      // (e.g. /clear); Awen keeps its own thread identity and resume cursor.
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const handleAskUserQuestion = Effect.fn("handleAskUserQuestion")(function* (
        ...
        // expected lookup key. See https://github.com/AllenReder/awen/issues/2388
      ...
      const handleResumeDialog = Effect.fn("handleResumeDialog")(function* (
        ...
        // The question copy lives in @awen/shared/claudeCompaction because
      ...
      const thinking = thinkingSupported
        ...
        : undefined;
      const ultrawen = isClaudeCatalogUltrawenEffort(effort);
      ...
      // A permission launch arg is folded into the mode Awen sends rather than
      ...
      const queryOptions: ClaudeQueryOptions = {
        ...
        // `ultrawen` is a Claude Code setting, not an API effort level. It is
        // normalized to `xhigh` above and paired with `settings.ultrawen`.
        ...(effectiveEffort
          ? {
  ...
  const rollbackThread: ClaudeAdapterShape["rollbackThread"] = Effect.fn("rollbackThread")(
    function* (threadId, numTurns) {
      ...
      // their Awen turn count agrees; steers must never be treated as extra turns.
      if (
