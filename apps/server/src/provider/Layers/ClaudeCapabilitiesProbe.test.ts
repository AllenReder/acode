...
import { ClaudeSettings } from "@awen/contracts";
...
it.layer(NodeServices.layer)("Claude capability probe SDK boundary", (it) => {
  it.effect("serializes strict no-MCP options and still resolves account capabilities", () =>
    Effect.gen(function* () {
      ...
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "awen-claude-probe-sdk-" });
      ...
      const workspaceCwd = yield* fs.makeTempDirectory({ prefix: "awen-claude-probe-cwd-" });
      ...
      yield* fs.writeFileString(
        ...
        [
          ...
          "}",
          "writeFileSync(process.env.AWEN_PROBE_INVOCATION_PATH, JSON.stringify({",
          "  args,",
          "  cwd: process.cwd(),",
          ...
          '          resolvedModel: "deepseek-v4.1-flash[1m]",',
          '          displayName: "deepseek-v4.1-flash[1M]",',
      ...
      const capabilities = yield* probeClaudeCapabilities(
        ...
        {
          ...process.env,
          AWEN_PROBE_INVOCATION_PATH: invocationPath,
          ENABLE_CLAUDEAI_MCP_SERVERS: "true",
        },
        workspaceCwd,
      );
