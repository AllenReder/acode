...
import { type ProviderApprovalDecision, type ProviderEvent, ThreadId } from "@awen/contracts";
  ...
  it.effect("looks up child model metadata once after activity registration", () =>
    Effect.gen(function* () {
      ...
      const runtime = yield* makeCodexSessionRuntime({
        ...
        environment: { ...process.env, AWEN_CODEX_COLLAB_SCRIPT: scriptPath },
      });
    ...
    it.live.skipIf(
      ...
      Effect.gen(function* () {
        ...
        const aliasDirectory = NodeFS.mkdtempSync(
          NodePath.join(NodeOS.tmpdir(), "awen-command-alias-"),
        );
        const aliasPath = NodePath.join(aliasDirectory, "awen-sleep");
