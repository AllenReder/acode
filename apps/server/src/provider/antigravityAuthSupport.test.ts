...
describe("Antigravity process environment", () => {
  const profile: AntigravityProfile = {
    platform: "linux",
    geminiHome: "/awen/userdata/providers/antigravity/profile",
    acpDirectory: "/awen/userdata/providers/antigravity/profile/antigravity-acp",
    tokenPath: "/awen/userdata/providers/antigravity/profile/antigravity-acp/acp_token.json",
    tempDirectory: "/awen/userdata/providers/antigravity/profile/antigravity-acp/tmp",
...
it.layer(NodeServices.layer)("Antigravity profile preparation", (it) => {
  it.effect("runs the browser helper with installed Node in standalone builds", () =>
    Effect.gen(function* () {
      ...
      expect(profile.browserCommand).not.toContain("/packaged/awen");
      expect(yield* fs.exists(profile.acpDirectory)).toBe(true);
    }).pipe(
      Effect.provideService(HostProcessIsExecutable, true),
      Effect.provideService(HostProcessExecutablePath, "/packaged/awen"),
    ),
  );
