...
import type { ProviderUserInputAnswers, UserInputQuestion } from "@awen/contracts";
...
/**
 * Client captured the plan for Awen's proposed-plan card. Abandon the native
 * Grok plan-approval gate so the turn unblocks; the user implements via Awen UI.
 */
 ...
 * Used so Awen can show the plan while plan mode is still active (before exit).
 */
...
export const makeXAiPromptCompletionRuntime = Effect.fn("makeXAiPromptCompletionRuntime")(
  function* (runtime: AcpSessionRuntime.AcpSessionRuntime["Service"]) {
    ...
    const allocatePromptFallbackId = Effect.sync(() => {
      ...
      return `awen-xai-prompt-${nextPromptFallbackId}`;
    });
