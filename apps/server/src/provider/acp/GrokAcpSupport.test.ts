...
describe("grokAcpSpawnArgs", () => {
  it("inherits the Grok CLI config when no Awen runtime mode is set", () => {
    expect(grokAcpSpawnArgs()).toEqual(["agent", "stdio"]);
  });

  it("forces Grok to ask when Awen is Supervised", () => {
...
describe("buildGrokAcpSpawnInput", () => {
  it("passes the Awen referrer through Grok OAuth env", () => {
