...
import type { AntigravityAuthMethod, ProviderInstanceId } from "@awen/contracts";
import { HostProcessPlatform } from "@awen/shared/hostProcess";
import { resolveNodeExecutable, nodeRuntimeUnavailableMessage } from "@awen/shared/nodeRuntime";
...
export const ANTIGRAVITY_AUTH_BROWSER_MARKER = "__AWEN_ANTIGRAVITY_AUTH_URL__";
...
const browserHelperSource =
  ...
  `()=>process.exit(0))`;
const browserPreflightUrl = "https://example.invalid/awen-antigravity-browser-preflight";
...
/** Recognizes native auth failures and interactive login blocked by Awen. */
...
function antigravityEnvironment(
  ...
  // Point it at a Awen-owned directory so the driver can reclaim the space.
...
/**
 * The agent reads its user-global skills under `GEMINI_HOME`, which Awen points
...
export const prepareAntigravityProfile = Effect.fn("prepareAntigravityProfile")(function* (input: {
  ...
  if (
    ...
    return yield* authSupportError(
      "The Awen runtime path cannot be used to suppress Antigravity browser launches.",
    );
  }
...
/** Receives native 1.1.1 sign-in URLs and Awen browser-helper URLs without logging stderr. */
