...
export interface TelemetryCounters {
  ...
  readonly allAwen: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allAwen: ResourceTelemetryAggregate;
  };
...
function applyLifecycleCounters(input: {
  ...
  let allAwen = input.counters.allAwen;
  for (const processDelta of input.deltas) {
    ...
    allAwen = incrementCounters(allAwen, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allAwen = incrementCounters(allAwen, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allAwen = incrementCounters(allAwen, { processExits: 1 });
  }

  return { backend, electron, monitor, allAwen };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allAwen: aggregate(ordered, counters.allAwen),
    },
    deltas: processDeltas,
  };
}
