...
import { hasProjectSettingsOverrides, resolveProjectSettings } from "@awen/shared/projectSettings";
...
import { GitManagerError, GitPullRequestMaterializationError } from "@awen/contracts";
...
import type { GitManagerServiceError } from "@awen/contracts";
import * as GitVcsDriver from "../vcs/GitVcsDriver.ts";
import * as SourceControlProviderRegistry from "../sourceControl/SourceControlProviderRegistry.ts";
import { detectPrTemplate } from "../sourceControl/PrTemplateDetection.ts";
import type { ChangeRequest } from "@awen/contracts";
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `awen/pr-${pullRequest.number}/${suffix}`;
}
...
export const make = Effect.gen(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(
      tempDir,
      `awen-pr-body-${process.pid}-${yield* randomUUIDv4(cwd)}.md`,
    );
