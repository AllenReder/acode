...
import { ANTIGRAVITY_DEFAULT_MODEL, ProviderInstanceId, ProviderSetupError } from "@awen/contracts";
...
const makeFixture = Effect.fn("makeAntigravityTextGenerationFixture")(function* (
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "awen-antigravity-text-test-" });
...
it.layer(NodeServices.layer)("AntigravityTextGeneration", (it) => {
  ...
  it.effect("uses the native default without sending Awen's default selection as a model ID", () =>
    Effect.gen(function* () {
      const fixture = yield* makeFixture();
