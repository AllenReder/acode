...
import { SpawnExecutableResolution } from "@awen/shared/shell";
...
it.effect("launches an installed editor with platform-safe arguments", () =>
  Effect.gen(function* () {
    ...
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "awen-editors-" });
...
// oxlint-disable-next-line awen/no-global-process-runtime -- the skip decision needs the real host platform, outside any Effect runtime.
it.skipIf(process.platform !== "win32")(
  "delivers the raw /select switch for spaced paths through real PowerShell",
  { timeout: 60_000 },
  async () => {
    const tempDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "awen-reveal-smoke-"));
    try {
  ...
  () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.gen(function* () {
        ...
        yield* launcher.launchEditor({
          editor: "file-manager",
          cwd: "/home/awen/workspace/media/clip.mp4",
          reveal: true,
        });
        return { kind, editors };
      }).pipe(
      ...
      assert.equal(
        decodedCommand,
        "$ProgressPreference = 'SilentlyContinue'; Start-Process 'explorer.exe' -ArgumentList ('/select,\"' + '\\\\wsl.localhost\\Ubuntu-24.04\\home\\awen\\workspace\\media\\clip.mp4' + '\"')",
      );
    }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
);
...
it.effect.skipIf(windowsHost)(
  ...
  () =>
    Effect.gen(function* () {
      ...
      assert.ok(launch);
      assert.deepEqual(launch.args, ["/home/awen/workspace/media"]);
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* launcher.launchEditor({
          editor: "file-manager",
          cwd: '/home/awen/work "quoted"/clip.mp4',
          reveal: true,
        });
      }).pipe(
        Effect.provide(
      ...
      assert.deepEqual(spawned.args, [
        '\\\\wsl.localhost\\Ubuntu-24.04\\home\\awen\\work "quoted"',
      ]);
    }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
);
      ...
      Layer.mergeAll(
        ...
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: {
              PATH: "C:\\awen-editor-discovery-cache-test",
              PATHEXT: ".COM;.EXE;.BAT;.CMD",
            },
          }),
        ),
    ...
    Effect.provide(
      Layer.mergeAll(
        ...
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: {
              PATH: "C:\\awen-editor-discovery-interrupt-test",
              PATHEXT: ".COM;.EXE;.BAT;.CMD",
            },
          }),
        ),
