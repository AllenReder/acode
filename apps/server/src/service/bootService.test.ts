...
const linuxRuntime = "/home/theo/.awen/runtime/versions/1.2.3/awen";
const linuxPlan = {
  program: [linuxRuntime, "__service-launcher"],
  baseDir: "/home/theo/.awen",
  logPath: "/home/theo/.awen/userdata/logs/boot-service.log",
  unitPath: "/home/theo/.config/systemd/user/awen.service",
};
...
it("reads the served Awen home back out of a rendered unit or plist", () => {
  const plan = (baseDir: string) => ({
    program: [`${baseDir}/runtime/versions/1.2.3/awen`, "__service-launcher"],
    baseDir,
  ...
  expect(
    BootService.bootServiceBaseDirOf(BootService.renderBootServiceUnit(plan("/home/theo/.awen"))),
  ).toBe("/home/theo/.awen");
  // Spaces and specifiers are quoted and escaped on the way in.
  expect(
    BootService.bootServiceBaseDirOf(
      BootService.renderBootServiceUnit(plan("/home/theo/Awen Data/100%")),
    ),
  ).toBe("/home/theo/Awen Data/100%");
  expect(
    BootService.bootServiceBaseDirOf(
...
const macRuntime = "/Users/theo/.awen/runtime/versions/1.2.3/awen";
const macPlan = {
  program: [macRuntime, "__service-launcher"],
  baseDir: "/Users/theo/.awen",
  logPath: "/Users/theo/.awen/userdata/logs/boot-service.log",
  unitPath: "/Users/theo/Library/LaunchAgents/com.allenreder.awen.service.plist",
};
...
it("appends both stdio streams to the boot service log", () => {
  ...
  expect(plist).toContain(
    "<key>StandardOutPath</key>\n  <string>/Users/theo/.awen/userdata/logs/boot-service.log</string>",
  );
  expect(plist).toContain(
...
it("escapes XML in host paths", () => {
  const plist = BootService.renderBootServicePlist(
    { ...macPlan, baseDir: "/Users/theo/Awen & <Co>" },
    { homeDir: "/Users/theo", environmentPath: "/Users/theo/Tools & <Scripts>:/usr/bin" },
  );

  expect(plist).toContain("<string>/Users/theo/Awen &amp; &lt;Co&gt;</string>");
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const home = yield* fs.makeTempDirectoryScoped({ prefix: "awen-boot-service-test-" });
  const baseDir = path.join(home, ".awen");
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable awen.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart awen.service") control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop awen.service" ||
        ...
        stdout:
          input.args[0] === "--version"
            ? // The runtime under test reports the version of the directory it
              ...
              `awen v${/versions\/([^/]+)\//.exec(input.command)?.[1] ?? "1.2.3"}\n`
    ...
    Effect.gen(function* () {
      ...
      Effect.provide(
        Layer.mergeAll(
          ...
          Layer.succeed(HostProcessExecutablePath, "/usr/bin/awen"),
          Layer.succeed(
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop awen.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now awen.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? ["systemctl --user stop awen.service", "systemctl --user restart awen.service"]
            : [
                "launchctl bootout --wait gui/501/com.allenreder.awen.service",
  ...
  it.effect("install with start=false rewrites the files and marks a restart pending", () =>
    Effect.gen(function* () {
      ...
      expect(yield* fs.readFileString(plan.unitPath)).toContain("versions/1.2.4/bin/awen");
      expect(
        commands.filter(
      ...
      // the reason is named so `awen service status` can point at restart.
  ...
  it.effect("restart stops and starts an installed service, and is a no-op otherwise", () =>
    Effect.gen(function* () {
      ...
      expect(
        ...
        "systemctl --user stop awen.service",
        "systemctl --user daemon-reload",
        "systemctl --user enable awen.service",
        "systemctl --user restart awen.service",
      ]);
    }),
  );

  it.effect("restart leaves a service that serves another Awen home alone", () =>
    Effect.gen(function* () {
      const { service, fs, commands, makeService } = yield* makeHarness();
      yield* service.install();
      commands.length = 0;
      const path = yield* Path.Path;
      const otherHome = yield* fs.makeTempDirectoryScoped({ prefix: "awen-other-home-" });

      const other = yield* makeService(undefined, "1.2.3", path.join(otherHome, ".awen"));
  ...
  it.effect("restarts without overwriting a pending remote update", () =>
    Effect.gen(function* () {
      ...
      for (const allowDowngrade of [false, true]) {
        ...
        ).toEqual(["systemctl --user stop awen.service", "systemctl --user restart awen.service"]);
      }
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "com.allenreder.awen.service.plist"),
        ),
      ).toBe(true);
      expect(yield* fs.readFileString(plan.unitPath)).toContain(
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/com.allenreder.awen.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.allenreder.awen.service",
        "launchctl enable gui/501/com.allenreder.awen.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/com.allenreder.awen.service";
