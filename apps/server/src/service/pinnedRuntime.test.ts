...
const version = "1.2.3";
const archiveName = `awen-server-${version}-linux-x64.tar.gz`;
  ...
  ProcessRunner.ProcessRunner.of({
    run: (input) =>
      Effect.gen(function* () {
        ...
        yield* fs
          .makeDirectory(path.join(stagingDir, "bin"), { recursive: true })
          .pipe(Effect.orDie);
        yield* fs
          .writeFileString(path.join(stagingDir, "bin", "awen"), "#!/bin/sh\n")
          .pipe(Effect.orDie);
        return {
          stdout: "",
          stderr: "",
...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("installs the verified release archive as the runtime executable", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "awen-pinned-archive-" });
      ...
      assert.equal(paths.entryPath, path.join(paths.versionDir, "bin", "awen"));
      ...
      assert.isFalse(yield* fs.exists(path.join(paths.versionDir, "awen-server-archive")));
    }),
  );
  ...
  it.effect("refuses an archive whose checksum does not match the release", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "awen-pinned-archive-bad-" });
      const commands: string[] = [];
      ...
      assert.equal(error.step, "verifying the Awen daemon release archive checksum");
  ...
  it.effect("validates a staging tree before atomically publishing it", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "awen-pinned-runtime-test-" });
  ...
  it.effect("replaces an incomplete pinned runtime", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "awen-pinned-runtime-repair-" });
      const finalPaths = pinnedRuntimePaths(path, baseDir, version, "linux");
  ...
  it.effect("removes staging when installation is interrupted", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "awen-pinned-runtime-interrupt-",
      });
