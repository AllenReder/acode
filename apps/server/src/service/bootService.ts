...
import { CLI_RELEASE_BASE_URL_ENV } from "@awen/shared/cliRelease";
...
const BOOT_SERVICE_NAME = "awen";
const BOOT_SERVICE_UNIT_FILE = `${BOOT_SERVICE_NAME}.service`;
// `.service` suffix keeps the label distinct from the desktop app's bundle id
// (com.allenreder.awen), so launchd and TCC records never collide.
const BOOT_SERVICE_LAUNCHD_LABEL = "com.allenreder.awen.service";
const BOOT_SERVICE_PLIST_FILE = `${BOOT_SERVICE_LAUNCHD_LABEL}.plist`;
const BOOT_SERVICE_UNIT_ENV = "AWEN_BOOT_SERVICE_UNIT";
...
/**
 * Reads `AWEN_HOME` back out of a rendered unit or plist. Only values this
...
export function bootServiceBaseDirOf(contents: string): string | undefined {
  const systemd = /^Environment=AWEN_HOME=(.*)$/m.exec(contents)?.[1];
  ...
  const plist = /<key>AWEN_HOME<\/key>\s*<string>([^<]*)<\/string>/.exec(contents)?.[1];
  if (plist !== undefined) {
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=AWEN_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
export class BootServiceInstallError extends Schema.TaggedError<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the Awen background service.";
  }
}
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    case "user-manager-unavailable":
      return "Cannot reach the systemd user manager. Run `systemctl --user status` in a login session for the service user. Install your distribution's systemd user-session support if it is missing; do not run Awen with sudo.";
    ...
    case "linger-disabled":
      return 'Lingering is disabled. Awen will stop when your last login session ends and will not start at boot. Run `sudo loginctl enable-linger "$(id -un)"` on this machine, then retry the service command as your normal user.';
    case "service-disabled":
      return "The service is not enabled to start automatically. Run `awen service install` to repair it.";
    case "service-stopped":
      return "The service is not running. Check the service log and `systemctl --user status awen.service`, then run `awen service install`.";
    case "restart-pending":
      return "A newer version is installed but the service is still running the previous one. Run `awen service restart` to switch.";
  }
}
...
export class BootServiceDowngradeRefusedError extends Schema.TaggedError<BootServiceDowngradeRefusedError>()(
  ...
  override get message(): string {
    return `Refusing to replace awen@${this.installedVersion} with older awen@${this.targetVersion}. Run the command again with --allow-downgrade to continue.`;
  }
}
...
export interface BootServiceStatus {
  ...
  /**
   * The Awen home the installed unit serves. The unit name is fixed per user,
   * so a caller working against another base dir must not treat this service
   * as its own; `awen update --base-dir` learned that by restarting the live
...
export class BootService extends Context.Service<
  BootService,
  {
    readonly install: (options?: {
       ...
       * is running now. `awen update` uses this when the user declines the
       * restart, so a later `awen service restart` lands on the new version.
       */
...
>()("@awen/server/service/bootService") {}
            ...
            Effect.mapError(
              (cause) =>
                new PinnedRuntimeInstallError({
                  step: "verifying the pinned awen runtime",
                  cause,
                }),
            ),
