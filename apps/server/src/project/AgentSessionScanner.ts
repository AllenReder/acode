...
>()("@awen/server/project/AgentSessionScanner") {}
...
/**
 * Awen runs its own agent sessions inside disposable worktrees. Their
 ...
 * worktrees directory plus the conventional `.awen/worktrees` layout, which
 * also catches sandboxes from other Awen homes on the same machine. Separators
...
function isAwenManagedWorktree(
  ...
  return (
    ...
    normalized.includes("/.awen/worktrees/")
  );
}
...
export const make = Effect.gen(function* () {
  ...
  const isExcludedProjectPath = (candidatePath: string) =>
    ...
    isAwenManagedWorktree(candidatePath, worktreesDir, foldWorktreeCase);
