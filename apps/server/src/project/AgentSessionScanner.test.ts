...
import { symlinksSupported } from "@awen/shared/testing/symlinks";
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        ServerConfig.layerTest(
          ...
          input.configBaseDir ?? { prefix: "awen-scanner-config-" },
        ),
...
it.layer(NodeServices.layer)("AgentSessionScanner", (it) => {
  describe("scan", () => {
    it.effect("reads Claude project cwds from transcripts, newest first", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("awen-claude-home-");
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const olderWorkspace = yield* makeTempDir("awen-workspace-older-");
        const newerWorkspace = yield* makeTempDir("awen-workspace-newer-");
    ...
    it.effect("groups Codex rollouts by cwd across date directories", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("awen-claude-home-");
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const workspace = yield* makeTempDir("awen-workspace-");
        const otherWorkspace = yield* makeTempDir("awen-workspace-other-");
    ...
    it.effect("returns the imported project ID through a realpath alias", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const workspace = yield* makeTempDir("awen-workspace-");
        const linkParent = yield* makeTempDir("awen-scanner-links-");
    ...
    it.effect("keeps case variants distinct when the filesystem identities differ", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("awen-claude-home-");
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const backingUpper = yield* makeTempDir("awen-backing-upper-");
        const backingLower = yield* makeTempDir("awen-backing-lower-");
        const aliasParent = yield* makeTempDir("awen-case-aliases-");
    ...
    it.effect("uses explicit provider instance homes instead of overridden legacy homes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("awen-claude-legacy-");
        const codexHomePath = yield* makeTempDir("awen-codex-legacy-");
        const claudeInstanceHome = yield* makeTempDir("awen-claude-instance-");
        const codexInstanceHome = yield* makeTempDir("awen-codex-instance-");
        const legacyWorkspace = yield* makeTempDir("awen-workspace-legacy-");
        const claudeWorkspace = yield* makeTempDir("awen-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("awen-workspace-codex-");
    ...
    it.effect("scans each distinct home across multiple instances once", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("awen-claude-home-");
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const otherCodexHome = yield* makeTempDir("awen-codex-other-");
        const workspace = yield* makeTempDir("awen-workspace-");
        const otherWorkspace = yield* makeTempDir("awen-workspace-other-");
    ...
    it.effect("honors provider instance home directory environment variables", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("awen-claude-legacy-");
        const codexHomePath = yield* makeTempDir("awen-codex-legacy-");
        const claudeEnvironmentHome = yield* makeTempDir("awen-claude-env-");
        const codexEnvironmentHome = yield* makeTempDir("awen-codex-env-");
        const claudeWorkspace = yield* makeTempDir("awen-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("awen-workspace-codex-");
    ...
    it.effect("does not scan provider instances disabled by the envelope or config", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("awen-claude-home-");
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const envelopeDisabledHome = yield* makeTempDir("awen-codex-disabled-envelope-");
        const configDisabledHome = yield* makeTempDir("awen-codex-disabled-config-");
        const envelopeWorkspace = yield* makeTempDir("awen-workspace-disabled-envelope-");
        const configWorkspace = yield* makeTempDir("awen-workspace-disabled-config-");
    ...
    it.effect("excludes the home directory, temporary root, and Awen data directory", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("awen-claude-home-");
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const configBaseDir = yield* makeTempDir("awen-scanner-base-");
        const workspace = yield* makeTempDir("awen-workspace-");
    ...
    it.effect("excludes Awen-managed worktree sandboxes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("awen-claude-home-");
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        ...
        const worktreeCwd = path.join(claudeHomePath, ".awen", "worktrees", "awen", "wt-1");
    ...
    it.effect("excludes Codex scratch directories and Downloads", () =>
      Effect.gen(function* () {
        ...
        const runId = path.basename(yield* makeTempDir("awen-scanner-test-"));
        ...
        const keep = yield* makeTempDir("awen-workspace-keep-");
    ...
    it.effect("skips linked git worktrees and reports the origin of real checkouts", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("awen-claude-home-");
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const repo = yield* makeTempDir("awen-workspace-repo-");
        const worktree = yield* makeTempDir("awen-workspace-worktree-");
        const plain = yield* makeTempDir("awen-workspace-plain-");
        const noRemote = yield* makeTempDir("awen-workspace-noremote-");
        const submodule = yield* makeTempDir("awen-workspace-submodule-");

        yield* fileSystem.makeDirectory(path.join(repo, ".git"));
        yield* fileSystem.writeFileString(
          path.join(repo, ".git", "config"),
          '[core]\n\tbare = false\n[remote "origin"]\n\turl = git@github.com:AllenReder/awen.git\n\tfetch = +refs/heads/*:refs/remotes/origin/*\n',
        );
        yield* fileSystem.writeFileString(
        ...
        assert.deepStrictEqual(
          result.candidates.map((candidate) => ({
            ...
            git:
              candidate.git == null
                ? null
                : {
                    remoteKey: candidate.git.remoteKey,
          ...
          [
            ...
            {
              path: repo,
              git: { remoteKey: "github.com/allenreder/awen", repository: "AllenReder/awen" },
            },
          ],
        );
      }),
    ...
    it.effect(
      "excludes sandboxes under the configured worktrees dir without .awen in the path",
      () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const claudeHomePath = yield* makeTempDir("awen-claude-home-");
          ...
          // dir contains no `.awen` segment — only the config-based prefix match
          // can exclude this one.
          const worktreeCwd = path.join(configBaseDir, "worktrees", "awen", "wt-2");
          yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
          yield* writeTranscript({
            filePath: path.join(claudeHomePath, "projects", "-slug", "a.jsonl"),
            contents: claudeSessionLine(worktreeCwd),
    ...
    it.effect("excludes sandboxes reached through a symlink into the worktrees dir", () =>
      Effect.gen(function* () {
        ...
        const worktreeCwd = path.join(configBaseDir, "worktrees", "awen", "wt-3");
        yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
    ...
    it.effect.each([64, 65])("shares metadata bytes across homes for %s one-MiB files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("awen-metadata-home-");
        const secondHome = yield* makeTempDir("awen-metadata-second-");
        const codexHomePath = yield* makeTempDir("awen-metadata-codex-");
        const firstWorkspace = yield* makeTempDir("awen-metadata-first-project-");
        const secondWorkspace = yield* makeTempDir("awen-metadata-second-project-");
        const directories = [
    ...
    it.effect.each([50, 51])("bounds metadata open/read calls for %s short-read files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("awen-short-metadata-home-");
        const codexHomePath = yield* makeTempDir("awen-short-metadata-codex-");
        const workspace = yield* makeTempDir("awen-short-metadata-project-");
    ...
    it.effect("bounds malformed metadata records without excluding another account", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("awen-record-metadata-home-");
        const secondHome = yield* makeTempDir("awen-record-metadata-second-");
        const codexHomePath = yield* makeTempDir("awen-record-metadata-codex-");
        const workspace = yield* makeTempDir("awen-record-metadata-project-");
        const directory = path.join(claudeHomePath, "projects", "p");
        const template = path.join(directory, "template.jsonl");
        yield* writeTranscript({
          filePath: template,
    ...
    it.effect.each([19_999, 20_000])(
      ...
      (count) =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const fileSystem = yield* FileSystem.FileSystem;
          const claudeHomePath = yield* makeTempDir("awen-directory-budget-home-");
          const codexHomePath = yield* makeTempDir("awen-directory-budget-codex-");
    ...
    it.effect("returns an empty result when neither home directory exists", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const root = yield* makeTempDir("awen-missing-homes-");
  ...
  describe("recentThreads", () => {
    it.effect.each([false, true])(
      ...
      (overflow) =>
        Effect.gen(function* () {
          ...
          const claudeHomePath = yield* makeTempDir("awen-record-limit-claude-");
          const codexHomePath = yield* makeTempDir("awen-record-limit-codex-");
          const workspace = yield* makeTempDir("awen-record-limit-project-");
    ...
    it.effect("keeps the provider instance that owns a custom session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("awen-claude-home-");
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const customHome = yield* makeTempDir("awen-codex-custom-");
        const workspace = yield* makeTempDir("awen-workspace-");
    ...
    it.effect("streams large transcripts across providers without hiding projects", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("awen-budget-claude-");
        const codexHomePath = yield* makeTempDir("awen-budget-codex-");
        const workspace = yield* makeTempDir("awen-budget-workspace-");
    ...
    it.effect("skips excessive records without blocking an older valid transcript", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("awen-record-budget-claude-");
        const codexHomePath = yield* makeTempDir("awen-record-budget-codex-");
        const workspace = yield* makeTempDir("awen-record-budget-workspace-");
        ...
        it.effect.skipIf(replacement === "symlink alias" && !symlinksSupported)(
          ...
          () =>
            Effect.gen(function* () {
              ...
              const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
              yield* TestClock.setTime(nowMs);
              const fixture = yield* makeTempDir("awen-replaced-cwd-");
    ...
    it.effect("checks file identity and provider before skipping completed history", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("awen-completed-claude-");
        const codexHomePath = yield* makeTempDir("awen-completed-codex-");
        const workspace = yield* makeTempDir("awen-completed-workspace-");
    ...
    it.effect("does not reopen a transcript that becomes a non-file after discovery", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const workspace = yield* makeTempDir("awen-workspace-");
        const nonFilePath = yield* makeTempDir("awen-non-file-");
    ...
    it.effect("does not import sessions from a Awen-managed worktree", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        ...
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const configBaseDir = yield* makeTempDir("awen-scanner-base-");
        const workspace = path.join(configBaseDir, "worktrees", "awen", "managed-worktree");
    ...
    it.effect("uses one deterministic provider instance for a shared session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("awen-claude-home-");
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const sharedHome = yield* makeTempDir("awen-codex-shared-");
        const workspace = yield* makeTempDir("awen-workspace-");
    ...
    it.effect("keeps a second account when the first has 5000 newer files", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("awen-claude-home-");
        const codexHomePath = yield* makeTempDir("awen-codex-home-");
        const oldWorkspace = yield* makeTempDir("awen-workspace-old-");
        const recentWorkspace = yield* makeTempDir("awen-workspace-recent-");
        const recentHome = yield* makeTempDir("awen-claude-recent-home-");
