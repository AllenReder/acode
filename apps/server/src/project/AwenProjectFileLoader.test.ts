...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(AwenProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "awen.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("AwenProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid awen.json", () =>
      Effect.gen(function* () {
        const loader = yield* AwenProjectFileLoader.AwenProjectFileLoader;
    ...
    it.effect("returns none when awen.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* AwenProjectFileLoader.AwenProjectFileLoader;
        const cwd = yield* makeTempDir;
