...
import { resolveGitWorktreePath, resolveWorktreeAwenHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "awen-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeAwenHome", () => {
  it.effect("answers with .awen before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeAwenHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".awen"));
