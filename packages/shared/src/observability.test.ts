    ...
    it.effect("flushes buffered trace records on close", () =>
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const tempDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "awen-trace-sink-" });
  ...
  it.each([
    {
      ...
      input: "authorization=Basic%20abc%3D%3D,x-tenant=awen",
      expected: { authorization: "Basic abc==", "x-tenant": "awen" },
    },
    {
