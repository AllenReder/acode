/**
 * `dist/bin.mjs` of the `awen` npm package: the entry point boot-service
 ...
 * executable in the sibling `@awen/awen-<platform>-<arch>` package.
 *
...
const executable = join(dirname(require.resolve("@awen/awen-" + process.platform + "-" + process.arch + "/package.json")), executableName);
...
const fail = (error) => {
  if (!error) return;
  process.stderr.write("awen: " + error.message + "\\n");
  child.kill("SIGTERM");
  process.exitCode = 1;
};
if (ipc) {
