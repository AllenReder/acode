...
// oxlint-disable-next-line awen/no-global-process-runtime -- This test launches a real host executable.
const hostPlatform = NodeOS.platform();
// oxlint-disable-next-line awen/no-global-process-runtime -- Match the real executable used by the subprocess.
...
it.skipIf(hostPlatform === "win32")(
  ...
  async () => {
    const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "awen-legacy-launcher-"));
    const entry = NodePath.join(root, "node_modules/awen/dist/bin.mjs");
    const executable = NodePath.join(
      root,
      `node_modules/@awen/awen-${hostPlatform}-${hostArch}/awen`,
    );
