...
/**
 * The environment's theme, set with `awen theme set <id>`. Each client applies
...
// Deliberately absent from ServerSettingsPatch: `awen theme set` checks that an
...
export const CodexSettings = makeProviderSettingsSchema(
  {
    ...
    shadowHomePath: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        providerSettingsForm: {
          placeholder: "~/.codex-awen/personal",
          clearWhenEmpty: "omit",
        },
      }),
    ),
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let Awen spawn the server when needed.",
        providerSettingsForm: {
...
export const ServerSettings = Schema.Struct({
   ...
   * withholds the MCP credential, so the `awen` server (and with it every
  ...
  /**
   * Whether this server may install and run Awen's device helper processes.
   ...
   * of the same value from one they already applied: `awen theme set` must act
