...
it.effect("decodes a thread creation result with its durable Awen session", () =>
  Effect.gen(function* () {
      ...
      pullRequests: [
        ...
        },
        {
          ...
          stack: {
            ...
            id: "7",
            number: 3,
            url: "https://github.com/AllenReder/awen/stacks/3",
            base: "main",
            layers: [
...
it.effect("project monograms validate text and palette colors", () =>
  Effect.gen(function* () {
    for (const text of ["A", "Awen", "É", "文書", "कि", "किखि", "e\u0301"]) {
      const projectIcon = {
        kind: "monogram",
        color: "violet",
        text,
    ...
    for (const projectIcon of [
      ...
      { kind: "monogram", text: "🚀", color: "blue" },
      { kind: "monogram", text: "Awen", color: "ultraviolet" },
    ]) {
      const result = yield* Effect.exit(
        decodeOrchestrationCommand({
          type: "project.meta.update",
...
it.effect("sends monograms as fallback icons that old and nightly clients can decode", () =>
  Effect.gen(function* () {
    ...
    for (const text of ["Awen", "क्ष्म", "e\u0301"]) {
