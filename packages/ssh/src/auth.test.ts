...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "awen-ssh-askpass-test-" });
      ...
      assert.equal(env.AWEN_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "awen");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\awen-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\awen-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
