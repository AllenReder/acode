...
const bearerRegistration = (target: BearerConnectionTarget) =>
...
describe("EnvironmentRegistry", () => {
  ...
  it.effect("persists and starts a newly registered environment", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        const registry = yield* EnvironmentRegistry.EnvironmentRegistry;
        yield* registry.register(bearerRegistration(BEARER_TARGET));
        yield* awaitConnectionState(
          registry,
  ...
  it.effect("a socket preflight rejection persists the connection as switched off", () =>
    Effect.gen(function* () {
      ...
      const harness = yield* makeHarness([BEARER_TARGET], [], [], { prepareError: error });
      yield* Effect.gen(function* () {
        const registry = yield* EnvironmentRegistry.EnvironmentRegistry;
        yield* registry.start;
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* registry.register(
          bearerRegistration(new BearerConnectionTarget({ ...BEARER_TARGET, label: "Renamed" })),
        );
        yield* Effect.yieldNow;
  ...
  it.effect("does not connect a persisted environment that was switched off", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        expect(entry?.enabled).toBe(false);
        expect((yield* registry.state(BEARER_TARGET.environmentId)).phase).toBe("available");
        expect(yield* Ref.get(harness.sessions)).toHaveLength(0);
      }).pipe(Effect.provide(harness.layer));
    }),
  );
  ...
  it.effect("keeps the runtime registered when removal fails", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness([BEARER_TARGET], [], [], {
  ...
  it.effect("gives a primary platform registration precedence over persisted registrations", () =>
    Effect.gen(function* () {
      ...
      const shadowedRegistration = bearerRegistration(shadowedTarget);
      const shadowedEntry = {
        target: shadowedTarget,
        profile: Option.some(shadowedRegistration.profile),
        enabled: true,
      };
