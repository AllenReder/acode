...
describe("EnvironmentSupervisor", () => {
  ...
  it.effect("keeps the latest failure visible throughout the next connection attempt", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness({
        prepare: (attempt) =>
          attempt === 1 ? Effect.fail(transient("Remote environment timed out.")) : Effect.never,
      });
      const supervisor = yield* EnvironmentSupervisor.make(TARGET_ENTRY, {
        initiallyDesired: true,
      }).pipe(Effect.provide(harness.dependencies));
      ...
      expect(retrying).toMatchObject({
        ...
        lastFailure: {
          ...
          reason: "transport",
          message: "Remote environment timed out.",
        },
      });
    }).pipe(Effect.provide(TestClock.layer())),
  );
  ...
  it.effect("does not lose an explicit disconnect among concurrent wakeup signals", () =>
    Effect.gen(function* () {
      ...
      yield* awaitState(supervisor.state, (state) => state.phase === "connected");
      yield* Effect.all([supervisor.disconnect, harness.wake("application-active")], {
