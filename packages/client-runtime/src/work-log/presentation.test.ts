...
import { ThreadId } from "@awen/contracts";
...
describe("resolveWorkEntryToolPresentation", () => {
  it.each([
    "mcp__awen__preview_click",
    "mcp__awen__preview_click",
    "mcp__awen__preview_click",
    "Awen.preview_click",
    "awen · preview_click completed",
    "awen/preview_click",
    "preview_click",
  ...
  it("labels device tools with the device icon", () => {
    expect(
      resolveWorkEntryToolPresentation({
        label: "mcp__awen__device_open",
        toolLifecycleStatus: "completed",
      }),
    ).toEqual({ displayName: "Opened a device in the Device panel", icon: "device" });
    expect(resolveWorkEntryToolPresentation({ label: "awen · device_screenshot" })).toEqual({
  ...
  it("uses structured MCP identity when the provider supplies a custom title", () => {
    expect(
      resolveWorkEntryToolPresentation({
        ...
        toolData: { server: "awen", tool: "preview_snapshot", result: { title: "Example" } },
      }),
  ...
  it("uses the summary's state only when the provider omitted a lifecycle status", () => {
    const entry = { label: "Awen.preview_click" };
  ...
  it.each([
    ...
    ["awen_thread_read", "Reading a Awen thread", "Read a Awen thread"],
    ["awen_thread_send", "Sending to a Awen thread", "Sent to a Awen thread"],
    [
      "awen_worktree_handoff",
  ...
  it("keeps Awen branding for non-browser tools and falls back to the original tool label", () => {
    expect(
      resolveWorkEntryToolPresentation({
        label: "mcp__awen__task_status",
        toolTitle: "Check the child task",
      }),
    ).toEqual({ displayName: "Getting delegated task status", icon: "awen" });
  });
...
describe("browser group summaries", () => {
  const browserEntry: WorkLogPresentationEntry = {
    label: "MCP tool call",
    toolData: { server: "awen", tool: "preview_click" },
  ...
  it("preserves first-seen action ordering alongside non-browser tools", () => {
    expect(
      summarizeToolGroup([
        ...
        {
          ...browserEntry,
          toolData: { server: "awen", tool: "task_status" },
        },
      ]),
      ...
      summarizeToolGroup([
        {
          ...
          toolData: { toolName: "mcp__awen__preview_evaluate" },
        },
      ]),
...
describe("resolveViewedImageAsset", () => {
  ...
  it("serves awen attachment paths in place like any other host path", () => {
    const path = "/Users/demo/.awen/dev/attachments/11111111-1111-4111-8111-111111111111.png";
...
describe("pull request tool presentation", () => {
  it.each([
    "mcp__awen__link_pull_request",
    "mcp__awen__link_pull_request",
    "Awen · link_pull_request",
    "awen/link_pull_request",
    "link_pull_request",
  ...
  it("recognizes unlink targets supplied as repository and number", () => {
    expect(
      resolveWorkEntryToolPresentation({
        ...
        toolLifecycleStatus: "completed",
        toolData: {
          toolName: "mcp__awen__unlink_pull_request",
  ...
  it("summarizes native PR work separately from ordinary tools and integration metadata", () => {
    const link: WorkLogPresentationEntry = {
      label: "Awen · link_pull_request",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolLifecycleStatus: "completed",
      toolSource: { key: "awen", name: "Awen", kind: "integration" },
    };
    const list: WorkLogPresentationEntry = {
      ...link,
      label: "Awen · list_thread_pull_requests",
    };
    expect(summarizeToolGroup([link, link, list])).toBe(
      "Linked 2 pull requests and checked linked pull requests",
    );
    expect(summarizeToolGroup([{ ...link, label: "Awen · unlink_pull_request" }])).toBe(
...
describe("device group summaries", () => {
  ...
  it("recognizes Claude tool names and preserves screenshot previews", () => {
    const entry = {
      ...
      toolData: { toolName: "mcp__awen__device_screenshot" },
  ...
  it("does not classify another server's tools as Awen device controls", () => {
    expect(
      summarizeToolGroup([
        {
          ...deviceEntry("device_open"),
