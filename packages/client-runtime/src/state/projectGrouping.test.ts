import { EnvironmentId, ProjectId } from "@awen/contracts";
...
const repositoryIdentity = {
  ...
  locator: {
    ...
    remoteName: "upstream",
    remoteUrl: "https://github.com/allenreder/awen.git",
  },
  provider: "github",
  owner: "allenreder",
  name: "awen",
...
describe("buildProjectGroups", () => {
  it("preserves every physical clone as a selectable member in repository modes", () => {
    const projects = [
      makeProject("awen", "/work/awen"),
      makeProject("awen-2", "/work/awen-2"),
      makeProject("awen-3", "/work/awen-3"),
    ];
  ...
  it("uses a shared custom title as the repository group's label", () => {
    const projects = [
      makeProject("first", "/work/awen", { title: "Custom project" }),
      makeProject("second", "/work/awen-2", { title: "Custom project" }),
    ];
  ...
  it("keeps the repository label when shared titles match its repository name", () => {
    const projects = [
      makeProject("first", "/work/awen", { title: "awen" }),
      makeProject("second", "/work/awen-2", { title: "awen" }),
    ];
  ...
  it("keeps physical clones in separate groups when requested", () => {
    ...
    expect(groups.map((group) => group.label)).toEqual(["awen", "awen-2", "awen-3"]);
  });

  it("applies a physical-project override without dropping its siblings", () => {
    const first = makeProject("awen", "/work/awen");
    const second = makeProject("awen-2", "/work/awen-2");
    const third = makeProject("awen-3", "/work/awen-3");
  ...
  it("dedupes stale registrations at one physical path using the freshest project", () => {
    const stale = makeProject("stale", "/work/awen", {
      repositoryIdentity: null,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const fresh = makeProject("fresh", "/work/awen/", {
  ...
  it("uses repository identity from a duplicate registration when the winner lacks it", () => {
    const identified = makeProject("identified", "/work/awen", {
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshUnidentified = makeProject("fresh", "/work/awen/", {
      repositoryIdentity: null,
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const sibling = makeProject("sibling", "/work/awen-2");
  ...
  it("uses the freshest winner's repository identity when stale duplicates disagree", () => {
    const staleIdentity = {
      ...repositoryIdentity,
      canonicalKey: "github.com/allenreder/old-repository",
      name: "old-repository",
  ...
  it("uses the freshest identity-bearing duplicate when the winner lacks identity", () => {
    ...
    const staleIdentified = makeProject("stale-identified", "/work/awen", {
      repositoryIdentity: staleIdentity,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshIdentified = makeProject("fresh-identified", "/work/awen/", {
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const winner = makeProject("winner", "/work/awen", {
      repositoryIdentity: null,
