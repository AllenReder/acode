...
- **Port Conflict**: `ACODE_DAEMON_PORT` is a requirement, not a preference. If that port is occupied, `daemon start` fails with `daemon-port-unavailable` and names the port instead of binding a different one, because the runtime descriptor and every client address the daemon by the configured port. `ACODE_PORT` (and `T3CODE_PORT`) stay preferences and still fall back to a free port when busy. Free the port or choose another number with `ACODE_DAEMON_PORT=<port>`.
