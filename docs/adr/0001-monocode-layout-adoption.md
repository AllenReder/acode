...
- **Rewrite from scratch.** Re-design `LayoutNode` and the operations with the same surface (`splitPane`, `removePane`, …) but ~200 lines scoped exactly to Awen's needs.
- **Adopt verbatim and keep everything.** Use Monocode's full `WorkspaceTab` model, including `editorPanes` and `terminalPanes`. Would force the Awen workbench into Monocode's notion of "tab" (one file-editing surface + N terminal panes), which is the wrong shape for C10.
...
- Strict-mode fixes (`noUncheckedIndexedAccess`) were applied to the kept code paths to match the Awen TypeScript baseline. These were the only behavioral edits — the geometry itself is unchanged.

## Consequences

- A future maintainer reading `layout.ts` will see comments noting the donor and what was pruned. Without this ADR they might assume the BSP code is original Awen work and reach for `git blame` expecting to find Awen authors.
...
- Strict-mode fixes (the `child === undefined` guards inside `for` loops) are Awen-specific patches on top of Monocode's code; they are necessary for `tsc --noEmit` to pass and do not change semantics. If we sync the file from upstream, these guards must be reapplied.
